...
const showAll = () => {
  itemsToShow.value = activities.value.length
}
</script>
...
<template>
  <div v-if="activities.length > 0">
    <table class="table-activities">
      ...
      <tbody>
        <tr v-for="item in limitedItems" :key="item.id_activity">
          ...
          <td>
            <router-link :to="{ name: 'ActivityDetail', params: { id: item.id_activity } }">
              Más detalles
            </router-link>
          </td>
        </tr>
    ...
    <div class="buttons-to-show">
      <button class="button-all" v-if="itemsToShow < activities.length" @click="showAll">
        Mostrar Todo
      </button>
