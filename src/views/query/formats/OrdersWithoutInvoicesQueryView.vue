...
const titleHeader = computed(() => {
  return isClosedTrip.value
    ? 'Órdenes sin Factura (Viajes Cerrados)'
    : 'Órdenes sin Factura (Viajes Activos)'
})

const fetchOrdersWithoutInvoices = async () => {
  isLoading.value = true
  try {
    const response = await queryService.getOrdersWithoutInvoices(isClosedTrip.value)
    orders.value = response.data
  } catch (err) {
    alertService.generalError(
      err.response?.data?.message?.error ||
        err.response?.data?.detail ||
        'Error al obtener las órdenes sin facturas.'
    )
  } finally {
...
onMounted(() => {
  fetchOrdersWithoutInvoices()
})

// Extract unique line names from fetched orders
const uniqueLines = computed(() => {
  const lines = orders.value.map((o) => o.line_name).filter(Boolean)
  return [...new Set(lines)].sort()
})

// Filtered orders for real-time search, line selection, and date limits
const filteredOrders = computed(() => {
  ...
  // Filter by start date limit (from delivery date)
  if (startDate.value) {
  ...
  // Filter by end date limit (up to delivery date)
  if (limitDate.value) {
    result = result.filter((o) => {
      if (!o.delivery_date) return false
...
// Metrics (computed over filtered results)
...
const formatDate = (dateString) => {
  ...
  // Shift by timezone offset to show correct local date
...
const exportToCSV = () => {
  ...
  link.setAttribute('download', `ordenes_sin_factura_${new Date().toISOString().slice(0, 10)}.csv`)
...
<template>
  <div>
    <div v-if="isLoading" class="global-loading-overlay">
      <p>Cargando órdenes sin facturas, por favor espere...</p>
    ...
    <div class="query-container">
      ...
      <div class="stats-grid">
        <div class="stat-card total">
          ...
          <div class="stat-info">
            ...
            <span class="stat-label">Órdenes sin Factura</span>
          </div>
        </div>
      ...
      <div class="table-container">
        <div v-if="filteredOrders.length === 0" class="no-results">
          No se encontraron órdenes sin facturas o ningún registro coincide con la búsqueda.
        </div>
